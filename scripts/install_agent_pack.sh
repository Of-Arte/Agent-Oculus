...
# Installs the local agent pack manifest into the Oculus profile home.
...
if command -v hermes >/dev/null 2>&1; then
  # Enable plugin in the Oculus profile (ignore if already enabled)
  hermes -p oculus plugins enable oculus >/dev/null 2>&1 || true
